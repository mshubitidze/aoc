let input = `FzQrhQpJtJMFzlpplrTWjTnTTrjVsVvvTnTs
...
//   .split("")
//   .map((i, idx) => (obj[i] = idx + 1));
// const split = (str, idx) => [str.slice(0, idx), str.slice(idx)];

// let sum1 = 0;

// for (let i = 0; i < arr.length; i++) {
//   let [first, last] = split(arr[i], arr[i].length / 2);
//   // console.log(first, last);
//   for (let j = 0; j < first.length; j++) {
//     if (last.includes(first[j])) {
//       obj[first[j]]
//         ? (sum1 += obj[first[j]])
//         : (sum1 += obj[first[j].toLowerCase()] + 26);
//       break;
//     }
//   }
// }
...
//   return result;
// };
// let arr1 = groupByN(3, arr).map((a) => a.map((b) => new Set(b)));
// // console.log("groups of three >>", arr1);
// let arr5 = [];
// for (let i = 0; i < arr1.length; i++) {
//   let arr4 = [];
//   for (let j = 0; j < arr1[i].length; j++) {
//     arr4.push(Array.from(arr1[i][j]).join(""));
//   }
//   arr5.push(arr4);
// }
// // console.log(arr5.sort((a, b) => a.length - b.length));
// let final = [];
// for (let arr of arr5) {
...
// let sum = 0;
// for (let i of final) {
//   obj[i] ? (sum += obj[i]) : (sum += obj[i.toLowerCase()] + 26);
// }
// console.log(sum);
